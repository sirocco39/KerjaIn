<div class="three gx-3 gy-3 col-12 col-lg-9 mt-3 px-md-1 ps-3 mt-lg-0 order-1 order-lg-1" style="height: 100vh;">
    ...
    @if ($selectedRoom)
    <div class="contain bg-light px-4 py-3 rounded-top-4 d-flex align-items-center" style="border: 1px solid #cacadd; max-height:56vh;">
    ...
    <div class="px-4 pb-0 chat-container container-fluid bg-light rounded-bottom-4 flex-fill flex-column justify-content-between" style="height:80%; border: 1px solid #cacadd;">
        ...
        <div id="chatMessages" class="chat-layout flex-fill overflow-auto" style="flex:6; height:85%;" wire:poll.5s>
            ...
            <div class="text-center small text-muted my-2">{{ \Carbon\Carbon::parse($date)->translatedFormat('d F Y') }}</div>
            @foreach ($group as $msg)
            <div class="d-flex {{ $msg->sender_id === Auth::id() ? 'justify-content-end' : 'justify-content-start' }} mb-2">
                <div class="bubble-message {{ $msg->sender_id === Auth::id() ? 'bubble-sender' : 'bubble-receiver' }}">
                    {{ $msg->message }}
            ...
            @endforeach
            @empty
            <div class="text-center text-muted mt-5">{{ __('ongoing.chat.belum_ada_pesan') }}</div>
            @endforelse
        </div>
        ...
        <div class="send-layout d-flex align-items-center justify-content-between" style="flex:1; height:10%;">
            <form wire:submit.prevent="send" class="d-flex flex-fill">
                @csrf
                <input type="text" class="me-2 form-control rounded-5 flex-grow-1" wire:model.defer="newMessage" placeholder="{{ __('ongoing.chat.placeholder_pesan') }}">
